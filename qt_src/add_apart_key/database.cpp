...
bool DataBase::createTable()
{
    ...
    if(!query.exec( "CREATE TABLE " TABLE " ("
                            ...
                            TABLE_COUNTRY     " VARCHAR(255)    NOT NULL,"
                            TABLE_UNIQ_KEY_APART       "     NOT NULL"
                        " )"
                    )){
...
{
    QString ekran_nik = "\"" + nik + "\"";

    if (
            search(ekran_nik.toStdString())
        )
        return false;
...
bool DataBase::search(std::string FindValue)
{
    std::string sWANT_VALUE = "loshad1";
    QSqlQuery query;

    std::string cmd =  "SELECT * FROM " TABLE " WHERE " TABLE_UNIQ_KEY_APART "="   "'"  + FindValue + "'";

    qDebug("zapros: %s ",cmd.c_str());
    ...
    qDebug() << "[DataBase::search] : " << query.first();
    return query.first();
}
